...
class Interface:
    """ Class contains addressing structures for particular TAP interface """

    def __init__(self, interface_name, interface_mac, logger):
        """ Class constructor """
        ...
        fcntl.ioctl(self.tap, TUNSETIFF, struct.pack("16sH", self.name, IFF_TAP | IFF_NO_PI))
        ...
        self.mac_unicast = interface_mac
        self.mac_multicast = []
        ...
        self.rx_ring = RxRing(self)
        self.tx_ring = TxRing(self)
...
class PacketHandler:
    ...
    def __init__(self, configured_interfaces):
        ...
        self.interface = [Interface(_, __, self.logger) for _, __ in configured_interfaces]
        ...
        if config.ip6_support:
            for interface in self.interface:
                ...
                interface.assign_ip6_multicast(IPv6Address("ff02::1"))
                # Create list of IPv6 unicast/multicast addresses stack should listen on
                interface.ip6_address_candidate = self.parse_stack_ip6_address_candidate(config.ip6_address_candidate, interface)
                self.create_stack_ip6_addressing(interface)
        ...
        if config.ip4_support:
            for interface in self.interface:
                ...
                ip4_address_dhcp = self.__dhcp4_client(interface)
                ip4_address_dhcp = [ip4_address_dhcp] if ip4_address_dhcp[0] else []
                interface.ip4_address_candidate = self.parse_stack_ip4_address_candidate(config.ip4_address_candidate + ip4_address_dhcp, interface)
                self.create_stack_ip4_addressing(interface)

        # Log all the addresses stack will listen on
        for interface in self.interface:
            self.logger.info(f"Interface {interface.name} listening on unicast MAC address: {interface.mac_unicast}")
            self.logger.info(f"Interface {interface.name} listening on multicast MAC addresses: {list(set(interface.mac_multicast))}")
            self.logger.info(f"Interface {interface.name} listening on brodcast MAC address: {interface.mac_broadcast}")

            if config.ip6_support:
                self.logger.info(f"Interface {interface.name} listening on unicast IPv6 addresses: {[str(_) for _ in interface.ip6_unicast]}")
                self.logger.info(f"Interface {interface.name} listening on multicast IPv6 addresses: {list(set(str(_) for _ in interface.ip6_multicast))}")

            if config.ip4_support:
                self.logger.info(f"Interface {interface.name} listening on unicast IPv4 addresses: {[str(_) for _ in interface.ip4_unicast]}")
                self.logger.info(f"Interface {interface.name} listening on multicast IPv4 addresses: {[str(_) for _ in interface.ip4_multicast]}")
                self.logger.info(f"Interface {interface.name} listening on brodcast IPv4 addresses: {[str(_) for _ in interface.ip4_broadcast]}")
    ...
    def __thread_packet_handler(self):
        ...
        while True:
            for interface in self.interface:
                self.phrx_ether(interface.rx_ring.dequeue())
    ...
    def parse_stack_ip6_address_candidate(self, configured_address_candidate, interface):
        """ Parse IPv6 candidate address list """

        valid_address_candidate = []

        for interface_name, address, gateway in configured_address_candidate:
    ...
    def create_stack_ip6_addressing(self, interface):
        """ Create lists of IPv6 unicast and multicast addresses stack should listen on """

        def __(ip6_address):
            if self.perform_ip6_nd_dad(ip6_address.ip):
                interface.assign_ip6_address(ip6_address)
                self.logger.debug(f"Succesfully claimed IPv6 address {ip6_address} on interface {interface.name}")
            else:
                self.logger.warning(f"Unable to claim IPv6 address {ip6_address} on interface {interface.name}")

        # Configure Link Local address(es) staticaly
        for ip6_address in list(interface.ip6_address_candidate):
            if ip6_address.is_link_local:
                interface.ip6_address_candidate.remove(ip6_address)
                __(ip6_address)

        # Configure Link Local address automaticaly
        if config.ip6_lla_autoconfig:
            ip6_address = IPv6Network("fe80::/64").eui64(interface.mac_unicast)
        ...
        if not self.ip6_address:
            self.logger.warning("Unable to assign any IPv6 link local address on interface {interface.name}, disabling IPv6 protocol")
            config.ip6_support = False
            return

        # Check if there are any statically configured GUA addresses
        for ip6_address in list(interface.ip6_address_candidate):
            interface.ip6_address_candidate.remove(ip6_address)
            __(ip6_address)
        ...
        if config.ip6_gua_autoconfig:
            ...
            for prefix, gateway in list(interface.icmp6_ra_prefixes):
                self.logger.debug(f"Attempting IPv6 address auto configuration for RA prefix {prefix}")
                ip6_address = prefix.eui64(interface.mac_unicast)
                ip6_address.gateway = gateway
                __(ip6_address)

    def parse_stack_ip4_address_candidate(self, configured_ip4_address_candidate, interface):
        """ Parse IPv4 candidate addresses configured in stack.py module """

        valid_address_candidate = []

        for interface_name, address, gateway in configured_ip4_address_candidate:
            if interface_name == interface.name:
                self.logger.debug(f"Parsing ('{address}', '{gateway}') entry")
                try:
                ...
                if gateway is not None:
                    try:
                        ...
                        if (
                            ...
                            or gateway == address.network_address
                            or gateway == address.broadcast_address
    ...
    def create_stack_ip4_addressing(self, interface):
        """ Create lists of IPv4 unicast, multicast and broadcast addresses stack should listen on """

        # Perform Duplicate Address Detection
        for _ in range(3):
            for ip4_unicast in [_.ip for _ in interface.ip4_address_candidate]:
    ...
    def send_arp_announcement(self, ip4_unicast, interface):
    ...
    def send_gratitous_arp(self, ip4_unicast, interface):
    ...
    def send_icmp6_multicast_listener_report(self, interface):
        ...
        if icmp6_mlr2_multicast_address_record := {
            ...
            for _ in interface.ip6_multicast
            if _ not in {IPv6Address("ff02::1")}
        }:
            self.phtx_icmp6(
                ip6_src=interface.ip6_unicast[0] if interface.ip6_unicast else IPv6Address("::"),
    ...
    def send_icmp6_nd_dad_message(self, ip6_unicast_candidate, interface):
    ...
    def send_icmp6_nd_router_solicitation(self, interface):
        """ Send out ICMPv6 ND Router Solicitation """

        self.phtx_icmp6(
            ip6_src=interface.ip6_unicast[0],
    ...
    def __dhcp4_client(self, interface):
        """ Obtain IPv4 address and default gateway using DHCP """

        def __send_dhcp_packet(dhcp_packet_tx, interface):
            socket.send_to(
                UdpMetadata(
