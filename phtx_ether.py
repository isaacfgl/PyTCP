...
def phtx_ether(self, child_packet, ether_src="00:00:00:00:00:00", ether_dst="00:00:00:00:00:00"):
    ...
    if ether_packet_tx.ether_type == ps_ether.ETHER_TYPE_IP6:
        ...
        if ip6_dst.is_multicast:
            ...
            self.logger.debug(f"{ether_packet_tx.tracker} - Resolved destiantion IPv6 {ip6_dst} to MAC {ether_packet_tx.ether_dst}")
            __send_out_packet()
            return
        ...
        for stack_ip6_address in self.ip6_address:
            if stack_ip6_address.ip == ip6_src and ip6_dst not in stack_ip6_address.network:
                ...
                if mac_address := self.icmp6_nd_cache.find_entry(stack_ip6_address.gateway):
                    ...
                    self.logger.debug(
                        f"{ether_packet_tx.tracker} - Resolved destiantion IPv6 {ip6_dst}"
    ...
    if ether_packet_tx.ether_type == ps_ether.ETHER_TYPE_IP4:
        ...
        if ip4_dst.is_limited_broadcast:
            ...
            self.logger.debug(f"{ether_packet_tx.tracker} - Resolved destiantion IPv4 {ip4_dst} to MAC {ether_packet_tx.ether_dst}")
            __send_out_packet()
            return
        ...
        for stack_ip4_address in self.ip4_address:
            if stack_ip4_address.ip == ip4_src and ip4_dst not in stack_ip4_address.network:
                ...
                if mac_address := self.arp_cache.find_entry(stack_ip4_address.gateway):
                    ether_packet_tx.ether_dst = mac_address
                    self.logger.debug(
                        f"{ether_packet_tx.tracker} - Resolved destiantion IPv4 {ip4_dst}"
                        + f" to Default Gateway MAC {ether_packet_tx.ether_dst}"
                    )
                    __send_out_packet()
                    return
